...
class FakherFleetAndroidApp(App):
    def build(self):
        ...
        # شريط العنوان (Header)
        header = BoxLayout(size_hint_y=0.1, bg_color=(0.17, 0.03, 0.05, 1))
        with header.canvas.before:
            Color(0.17, 0.03, 0.05, 1)
        ...
        # شبكة الـ 40 مفتاحاً (5 صفوف × 8 أعمدة أو تمرير)
        ...
        for btn_info in self.buttons_data:
            btn = Button(
                ...
                bold=True,
                background_normal='',
                background_color=(0.1, 0.26, 0.2, 1),
                color=(1, 1, 1, 1),
                size_hint_y=None,
                height=110,
                halign='center',
    ...
    def on_key_click(self, btn_info):
        ...
        lbl = Label(
            text=f"تم الضغط على:\n{btn_info['title']}\n\nالقسم جاهز للربط بالخدمات السحابية/Firebase.",
            halign='center'
        )
        close_btn = Button(text="إغلاق", size_hint_y=0.3, background_color=(0.8, 0.2, 0.2, 1))
